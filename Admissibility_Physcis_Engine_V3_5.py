...
"""
================================================================================
MASTER VERIFICATION ENGINE â€” FCF v3.5
================================================================================
...
def _gravity_pre_closure_theorems() -> Dict[str, Any]:
    ...
    return {
        ...
        'T8': {
            ...
            'epistemic': 'P_structural',
            'summary': (
                'd = 4 uniquely selected: d\u22643 excluded by A4 (no propagating '
                'graviton \u2192 no geometric records). d\u22655 excluded by A1+Lovelock '
                '(non-unique response law exceeds single-channel capacity). '
                'd = 4 is the unique survivor.'
            ),
            'key_result': 'd = 4 spacetime dimensions',
            'dependencies': ['T_gauge', 'A1', 'A4'],
        },
        'T9_grav': {
...
AXIOMS = {'A1', 'A3', 'A4'}  # v3.5: L_nc (was A2), L_col (was A5) are derived lemmas
DERIVED_LEMMAS = {'L_nc', 'L_col'}

def validate_dependencies(all_results: Dict[str, Any]) -> Dict[str, Any]:
    """Check that every theorem's dependencies are satisfied."""
    known_ids = set(all_results.keys()) | AXIOMS | DERIVED_LEMMAS | {
        'Regime assumption', 'T8 (d=4)', 'T_channels',
        'Δ_geo closure', 'T3', 'T_gauge', 'T7',
        'M', 'NT',  # Structural postulates for L_nc
    }
    ...
    for tid, r in all_results.items():
        for dep in r.get('dependencies', []):
            ...
            if dep_clean not in known_ids and dep not in known_ids:
                ...
                if not any(dep.startswith(a) for a in AXIOMS | DERIVED_LEMMAS):
...
""")
    ...
    print("""
  AXIOMS A1â€“A4 + LEMMAS L_nc, L_col
      â”‚
...
GAP_REGISTRY = {
    ...
    'T1': {
        ...
        'gap': 'IMPORT. Framework proves L_nc (non-closure) â†’ incompatible observables. KS theorem '
    ...
    'T2': {
        ...
        'gap': 'IMPORT. State existence now PROVED: L_nc â†’ non-trivial enforcement â†’ '
               ...
               'now derived from A1+L_nc. Imports: GNS (1943), Kadison (1951).',
    ...
    'T_Îº': {
        'anchor': 'A4 (backward) + A1+A4 (forward) uniqueness proof',
        'gap': 'CLOSED. Full proof (7 steps): Îº â‰¥ 2 from independent forward (A1+A4: persistence requires stabilization) '
               'and backward (A4) commitments. Îº â‰¤ 2 because only A1+A4 '
    ...
    'T4': {
        ...
        'gap': 'IMPORT. L_nc (non-closure) forces chiral fermions in anomaly-free '
    ...
    'T6': {
        ...
        'gap': 'IMPORT. The Î²-function FORM (capacity competition drives running) '
               'is derived from A1 + L_col. The 1-loop COEFFICIENTS (b_i) are imported '
    ...
    'T8': {
        ...
        'gap': 'CLOSED. d â‰¤ 3 hard-excluded: d=3 has 0 propagating graviton DOF '
               ...
               '(C_ext/C_int = 0.5). dâ‰¥5 excluded by A1+Lovelock (non-unique response law in d\u22655 '
               'exceeds single-channel capacity budget). '
               'Upgrade: "optimal" â†’ "minimal admissible under A1+A3+A4."',
    ...
    'Î“_closure': {
        ...
        'gap': 'CLOSED. All sub-theorems closed. Î“_ordering (closed), '
               ...
               'R2 for event localization, A1+Lovelock for dâ‰¥5 exclusion, external imports.',
...
def display_audit_gaps(master: Dict[str, Any]):
    ...
    print(f"\n{'â•' * W}")
    print(f"  AUDIT-GAPS REPORT â€” FCF v3.5")
