...
"""
================================================================================
FCF THEOREM BANK -- v4.3 CANONICAL
================================================================================
...
TIER 3F: Flavor Mixing (L_gen_path, T_capacity_ladder, L_D2q, L_H_curv,
         T_q_Higgs, L_holonomy_phase, L_adjoint_sep, L_channel_crossing,
         T_CKM, T_PMNS_partial)
TIER 4: Gravity & Dark Sector (+ L_equip)
TIER 5: Delta_geo Closure

v4.2 CANONICAL: The Canonical Object (Theorem 9.16, Paper 13 Â§9).
...
v4.2.3: Paper 2 audit â€” 3 missing theorems added.
  ...
  P_exhaust ADDED: Predicate exhaustion â€” Q1+Q2 unique MECE partition.
...
v4.3.0: Flavor mixing tier -- CKM from zero parameters.
  10 NEW THEOREMS added (Tier 3F: Flavor Mixing).
  L_gen_path [P]: Generation path graph (Hasse of total order).
  T_capacity_ladder [P]: Q(g) quadratic ladder, q_B = (7,4,0).
  L_D2q [P]: Universal second difference D2q = -eps.
  L_H_curv [P | L_eps*]: Interior bump h=(0,1,0) from l1 on path.
  T_q_Higgs [P_structural]: q_H = (7,5,0) = q_B + h.
  L_holonomy_phase [P_structural]: phi = pi/4 from SU(2) holonomy.
  L_adjoint_sep [P]: Dk = 3 = dim(adj SU(2)).
  L_channel_crossing [P_structural]: c_Hu/c_Hd = x^3.
  T_CKM [P_structural]: Zero-parameter CKM, 6/6 within 5%.
  T_PMNS_partial [P_structural | open]: PMNS structural wall.
    Neutrino FN texture rank-1 -> theta_12 solver-dependent.
    theta_23 ~ 44 deg and theta_13 ~ 8 deg solver-stable.
    PMNS >> CKM correctly predicted from no-color.
  _eigh ADDED: Hermitian eigensystem (two-step Jacobi, pure Python).
  DAG fully acyclic. 89 entries (1 axiom + 2 postulates + 86 theorems).
...
def check_A1():
    ...
    CONSEQUENCES (through the derivation chain):
      ...
      - Operator algebra (T2): finite-dim witness Ã¢â€ â€™ GNS Ã¢â€ â€™ Hilbert space
      - Gauge structure (T3): local enforcement Ã¢â€ â€™ automorphism Ã¢â€ â€™ gauge
      - Bekenstein bound (T_Bek): finite interface Ã¢â€ â€™ area law
      - Everything else follows through the DAG

    STATUS: AXIOM Ã¢â‚¬â€ not derived, not derivable. This is the single
    physical input of the framework.
    """
    from fractions import Fraction

    # A1 is not proved Ã¢â‚¬â€ it IS the starting point.
    ...
    return _result(
        ...
        summary=(
            ...
            'specific value of C Ã¢â‚¬â€ only finiteness and positivity matter.'
        ),
        key_result='Finite enforcement capacity exists (C > 0, C < infinity)',
        dependencies=[],  # no dependencies Ã¢â‚¬â€ this is the root
        artifacts={
            'type': 'axiom',
...
def check_M():
    ...
    Used only by L_loc (locality derivation). M + NT + A1 Ã¢â€ â€™ locality.

    STATUS: POSTULATE Ã¢â‚¬â€ not derived from A1.
    """
    from fractions import Fraction
    ...
    # Any partition works Ã¢â‚¬â€ M just says partition exists
    C_1 = Fraction(1)
    C_2 = C_total - C_1
...
def check_NT():
    ...
    and no asymmetry can develop Ã¢â‚¬â€ no gauge group selection, no
...
def check_T3():
    ...
    DR APPLICABILITY NOTE (red team v4 canonical):
      ...
      algebraic QFT framework, which classically assumes PoincarÃƒÂ©
      covariance. However, the DR reconstruction theorem's core mechanism
      Ã¢â‚¬â€ recovering a compact group from its symmetric tensor category of
      representations Ã¢â‚¬â€ is purely algebraic (Tannaka-Krein duality).
      ...
      What DR does NOT need for the structural consequence we use:
        (d) PoincarÃƒÂ© covariance: this determines HOW the gauge field transforms
      ...
      The causal poset from L_irr serves as the index set; full PoincarÃƒÂ©
...
def check_L_count():
    """L_count: Capacity Counting Ã¢â‚¬â€ 1 structural enforcement channel = 1 unit.
    ...
    Step 3 Ã¢â‚¬â€ Structural vs kinematic DOF:
      ...
      element of the enforcement algebra:
        ...
        (b) T_field [P]: chiral species are independently enforceable presences.
            ...
            Count: 15 per generation Ãƒâ€” 3 generations = 45.
    ...
    Step 4 Ã¢â‚¬â€ Independence (L_loc + T_M [P]):
      ...
      enforcement resources Ã¢â‚¬â€ the counting is additive.

    Step 5 Ã¢â‚¬â€ Minimality (T_kappa + L_epsilon* [P]):
    ...
    WHY NOT count polarizations/helicities:
      ...
      Counting polarizations would give 12Ãƒâ€”2 = 24 gauge DOF, yielding
      C_total = 73 and Omega_Lambda = 54/73 = 0.740 (obs: 0.689, 7.4% off).
      The structural counting gives 61 and 0.05% match Ã¢â‚¬â€ this is not
    ...
    STATUS: [P] Ã¢â‚¬â€ follows from L_epsilon*, T_kappa, T3, T_field, T_M, L_loc.
    """
    from fractions import Fraction
    ...
    # ================================================================
    # Step 5: Minimality check Ã¢â‚¬â€ each is exactly 1 distinction
    # ================================================================
    # kappa = 2: each distinction locks exactly 2 states
    kappa = 2
    # Total states locked at saturation
    states_locked = C_total * kappa
    assert states_locked == 122, "61 distinctions Ãƒâ€” 2 states = 122"
    ...
    # Cross-check: the 16 = 5 multiplet types Ãƒâ€” 3 gens + 1 Higgs
    # also equals dim(G) + dim(Higgs) = 12 + 4 = 16
    n_mult_refs = 5 * n_gen + 1  # 5 multiplet types Ãƒâ€” 3 + 1 Higgs
...
def check_L_equip():
    """L_equip: Horizon Equipartition ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â capacity fractions = energy density fractions.

    STATEMENT: At the causal horizon (Bekenstein saturation), each capacity
    unit contributes equally to ÃƒÂ¢Ã…Â¸Ã‚Â¨T_ÃƒÅ½Ã‚Â¼ÃƒÅ½Ã‚Â½ÃƒÂ¢Ã…Â¸Ã‚Â©, so ÃƒÅ½Ã‚Â©_sector = |sector| / C_total.

    PROOF (4 steps, all from [P] theorems):

    Step 1 (A4 + T_entropy [P]):
      Irreversibility ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ entropy increases monotonically.
      At the causal horizon (outermost enforceable boundary), entropy
      is maximized: ÃƒÂÃ‚Â_horizon = argmax S(ÃƒÂÃ‚Â) subject to ÃƒÅ½Ã‚Â£ÃƒÅ½Ã‚Âµ_i = C.

    Step 2 (L_ÃƒÅ½Ã‚Âµ* [P]):
      Each distinction costs ÃƒÅ½Ã‚Âµ_i ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ ÃƒÅ½Ã‚Âµ > 0 (minimum enforcement cost).
      Distinctions are discrete: C_total = ÃƒÂ¢Ã…â€™Ã…Â C/ÃƒÅ½Ã‚ÂµÃƒÂ¢Ã…â€™Ã¢â‚¬Â¹ units.
      Total capacity C = C_totalÃƒâ€šÃ‚Â·ÃƒÅ½Ã‚Âµ + r, where 0 ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¤ r < ÃƒÅ½Ã‚Âµ.

    Step 3 (T_entropy [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â Lagrange multiplier / max-entropy):
      Maximize S = -ÃƒÅ½Ã‚Â£ p_i ln p_i subject to ÃƒÅ½Ã‚Â£ÃƒÅ½Ã‚Âµ_i = C and ÃƒÅ½Ã‚Âµ_i ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ ÃƒÅ½Ã‚Âµ.
      Unique solution (by strict concavity of S): ÃƒÅ½Ã‚Âµ_i = C/C_total for all i.
    ...
    Step 4 (Ratio independence):
      With ÃƒÅ½Ã‚Âµ_i = C/C_total for all i:
        E_sector = |sector| ÃƒÆ’Ã¢â‚¬â€ (C/C_total)
        ÃƒÅ½Ã‚Â©_sector = E_sector / E_total = |sector| / C_total
      The result is INDEPENDENT of C, ÃƒÅ½Ã‚Âµ, and the surplus r.
      Only the COUNT matters. ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â¡

    COROLLARY: The cosmological budget ÃƒÅ½Ã‚Â©_ÃƒÅ½Ã¢â‚¬Âº = 42/61, ÃƒÅ½Ã‚Â©_m = 19/61,
    f_b = 3/19 follow from [P]-counted sector sizes alone.
    No regime assumptions (R12.0/R12.1/R12.2) required.

    STATUS: [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all steps use proved theorems or axioms.
    """
    ...
    for r_frac in [Fraction(0), Fraction(1, 10), Fraction(1, 2), Fraction(99, 100)]:
        ...
        assert eps_eff >= eps, f"Effective cost must be ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â¥ ÃƒÅ½Ã‚Âµ"
        ...
        for name, count in sectors.items():
            ...
            assert omega == Fraction(count, C_total), (
                f"ÃƒÅ½Ã‚Â©_{name} must equal {count}/{C_total} for any r, "
                f"got {omega} at r={r_frac}"
            )

    # Verify the MECE partition (binary dichotomies)
    # Level 1: distinguishable information? YESÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢matter(19), NOÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢vacuum(42)
    matter = sectors['baryon'] + sectors['dark']
    vacuum = sectors['vacuum']
    assert matter + vacuum == C_total, "Level 1 exhaustive"

    # Level 2: conserved flavor QN? YESÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢baryon(3), NOÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢dark(16)
    assert sectors['baryon'] + sectors['dark'] == matter, "Level 2 exhaustive"

    # Cross-check: two independent routes to 16
    N_mult = 5 * 3 + 1  # 5 multiplet types ÃƒÆ’Ã¢â‚¬â€ 3 gens + 1 Higgs
    ...
    return _result(
        ...
        summary=(
            ...
            'capacity surplus uniformly over C_total discrete units (L_ÃƒÅ½Ã‚Âµ*). '
            'Uniform distribution preserves count fractions: '
            'ÃƒÅ½Ã‚Â©_sector = |sector|/C_total exactly, independent of '
            'total capacity C and surplus r. '
            'Replaces regime assumptions R12.0/R12.1/R12.2 with derivation. '
            'Algebraically verified: ratio invariant for all r ÃƒÂ¢Ã‹â€ Ã‹â€  [0, ÃƒÅ½Ã‚Âµ).'
        ),
        key_result='ÃƒÅ½Ã‚Â©_sector = |sector|/C_total at Bekenstein saturation (proved)',
        ...
        artifacts={
            ...
            'surplus_invariance': 'verified for r ÃƒÂ¢Ã‹â€ Ã‹â€  {0, 1/10, 1/2, 99/100}',
...
def check_T21a():
    ...
    UPGRADE HISTORY: [P_structural] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P] (corollary of T21b [P]).
    STATUS: [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â direct corollary of analytic Lyapunov proof.
    """
    # T21b proves w(s) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w* globally. Then p = w1/(w1+w2) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w1*/(w1*+w2*) = 3/13.
    from fractions import Fraction
    ...
    return _result(
        ...
        summary=(
            ...
            'Since w(s) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w* globally (T21b [P], analytic Lyapunov), '
            'p(s) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ p* = 3/13. Upgrade: [P_structural] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P].'
        ),
        key_result='p(s) = w(s)/W(s) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ p* = 3/13 (non-autonomous share dynamics)',
        dependencies=['T21b'],
    )


def check_T21b():
    """T21b: Lyapunov Stability (RG Attractor) ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â ANALYTIC PROOF.
    ...
    The system is a competitive Lotka-Volterra ODE:
      dw_i/ds = w_i(-ÃƒÅ½Ã‚Â³_i + ÃƒÅ½Ã‚Â£_j a_ij w_j)
    
    Standard Lyapunov function:
      V(w) = ÃƒÅ½Ã‚Â£_i (w_i - w_i* - w_i* ln(w_i/w_i*))
    
    V(w*) = 0, V(w) > 0 for all w ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â  w* in RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â  (Jensen's inequality).
    
    Time derivative:
      dV/ds = ÃƒÅ½Ã‚Â£_i (1 - w_i*/w_i)(dw_i/ds)
            = ÃƒÅ½Ã‚Â£_i (w_i - w_i*)(-ÃƒÅ½Ã‚Â³_i + ÃƒÅ½Ã‚Â£_j a_ij w_j)
            = ÃƒÅ½Ã‚Â£_i (w_i - w_i*) ÃƒÅ½Ã‚Â£_j a_ij (w_j - w_j*)   [using ÃƒÅ½Ã‚Â³_i = ÃƒÅ½Ã‚Â£_j a_ij w_j*]
            = (w - w*)ÃƒÂ¡Ã‚ÂµÃ¢â€šÂ¬ A (w - w*)
    
    Competition matrix A = [[1, 1/2], [1/2, 13/4]] is symmetric positive definite:
      det(A) = 1ÃƒÆ’Ã¢â‚¬â€(13/4) - (1/2)Ãƒâ€šÃ‚Â² = 3 > 0
      trace(A) = 1 + 13/4 = 17/4 > 0
    
    Therefore dV/ds > 0 for all w ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â  w*:
      Forward flow (IR): V increases ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w* is UNSTABLE (IR repeller)
      Reverse flow (UV): V decreases ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w* is GLOBALLY STABLE (UV attractor)
    
    Basin of attraction = entire positive orthant RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â .
    
    UPGRADE HISTORY: [P_structural | numerical] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P] (analytic Lyapunov).
    STATUS: [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â standard Lotka-Volterra stability, A sym pos def.
    """
    from fractions import Fraction
    
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Competition matrix (from T22 [P]) ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    x = Fraction(1, 2)
    a11 = Fraction(1)
    a12 = x            # = 1/2
    a21 = x            # symmetric
    ...
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Verify symmetric positive definite ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    ...
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Fixed point (from T21 + T22 + T27d) ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    gamma1, gamma2 = Fraction(1), Fraction(17, 4)
    # ÃƒÅ½Ã‚Â³_i = ÃƒÅ½Ã‚Â£_j a_ij w_j* ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ solve linear system
    ...
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Verify fixed point satisfies Aw* = ÃƒÅ½Ã‚Â³ ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    assert a11 * w1_star + a12 * w2_star == gamma1, "FP eq 1"
    assert a21 * w1_star + a22 * w2_star == gamma2, "FP eq 2"
    
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Verify sinÃƒâ€šÃ‚Â²ÃƒÅ½Ã‚Â¸_W ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    r_star = w1_star / w2_star
    sin2 = r_star / (1 + r_star)
    assert sin2 == Fraction(3, 13), "Must give sinÃƒâ€šÃ‚Â²ÃƒÅ½Ã‚Â¸_W = 3/13"
    
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Lyapunov proof verification ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    # dV/ds = (w-w*)ÃƒÂ¡Ã‚ÂµÃ¢â€šÂ¬ A (w-w*) > 0 for all w ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â  w*
    ...
    # ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ Numerical cross-check (still valuable for confidence) ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬ÃƒÂ¢Ã¢â‚¬ÂÃ¢â€šÂ¬
    ...
    for w10, w20 in test_ics:
        ...
        s2 = r/(1+r)
        assert abs(s2 - 3/13) < 0.01, f"IC ({w10},{w20}): sinÃƒâ€šÃ‚Â²ÃƒÅ½Ã‚Â¸_W={s2:.4f} ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â  3/13"
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'ANALYTIC PROOF: V(w) = ÃƒÅ½Ã‚Â£(w_i - w_i* - w_i* ln(w_i/w_i*)) is '
            'Lyapunov function. dV/ds = (w-w*)ÃƒÂ¡Ã‚ÂµÃ¢â€šÂ¬ A (w-w*) > 0 since A is '
            'symmetric positive definite (det=3, trace=17/4). '
            'w* = (3/8, 5/4) is globally stable UV attractor. '
            'Basin = entire RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â . Upgrade: [P_structural] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P].'
        ),
        key_result='V(w) Lyapunov: A sym pos def (det=3) ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ w* global attractor (analytic proof)',
...
def check_T21c():
    ...
    The basin of attraction of w* is the entire positive orthant RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â .
    No alternative attractors, limit cycles, or escape trajectories exist.
    
    PROOF: T21b provides V(w) with V(w*) = 0, V > 0 elsewhere, and
    dV/ds = (w-w*)ÃƒÂ¡Ã‚ÂµÃ¢â€šÂ¬ A (w-w*) > 0 for all w ÃƒÂ¢Ã¢â‚¬Â°Ã‚Â  w* (A sym pos def).
    A global Lyapunov function with unique minimum ÃƒÂ¢Ã…Â¸Ã‚Â¹ unique global attractor.
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'Basin = entire positive orthant RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â . '
            ...
            'Upgrade: [P_structural] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P].'
        ),
        key_result='Basin = entire positive orthant RÃƒâ€šÃ‚Â²ÃƒÂ¢Ã¢â‚¬Å¡Ã…Â  (no alternative attractors)',
        dependencies=['T21b'],
    )
...
def check_T11():
    ...
    Three-step derivation:
      Step 1: Global admissibility != sum of local admissibilities (from L_nc).
              Some correlations are globally locked ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â admissible, enforced,
              irreversible, but not attributable to any finite interface.

      Step 2: Global locking necessarily gravitates (from T9_grav).
              Non-redistributable correlation load ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ uniform curvature
              pressure ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ cosmological constant.
      ...
      Step 4 (L_equip [P]): At Bekenstein saturation, each capacity unit
              contributes equally to ÃƒÂ¢Ã…Â¸Ã‚Â¨T_ÃƒÅ½Ã‚Â¼ÃƒÅ½Ã‚Â½ÃƒÂ¢Ã…Â¸Ã‚Â©. Therefore:
              ÃƒÅ½Ã‚Â©_ÃƒÅ½Ã¢â‚¬Âº = C_vacuum / C_total = 42/61 = 0.6885 (obs: 0.6889, 0.05%).

    UPGRADE HISTORY: [P_structural | structural_step] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P] via L_equip.
    STATUS: [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â mechanism + quantitative prediction both derived.
    """
    ...
    return _result(
        ...
        summary=(
            ...
            'Non-redistributable load ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ uniform curvature (cosmological '
            'constant). Lambda > 0 from positive enforcement cost. '
            'Quantitative: ÃƒÅ½Ã‚Â©_ÃƒÅ½Ã¢â‚¬Âº = 42/61 = 0.6885 (obs: 0.6889, 0.05%) '
            'via L_equip (horizon equipartition). '
            'Upgrade: [P_structural] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P] via L_equip.'
        ),
        key_result='ÃƒÅ½Ã‚Â©_ÃƒÅ½Ã¢â‚¬Âº = 42/61 = 0.6885 (obs: 0.6889, error 0.05%)',
        dependencies=['T9_grav', 'T4F', 'T_field', 'T_gauge', 'T_Higgs', 'A1', 'L_equip', 'T12E', 'L_count'],
        artifacts={
            'mechanism': 'global locking ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ uniform curvature',
...
def check_T12E():
    ...
    Derivation:
      ...
      Bridge (L_equip [P]):
        ...
        ÃƒÅ½Ã‚Â©_sector = |sector| / C_total EXACTLY, for any surplus r.
    ...
    STATUS: [P] ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â all counts from [P] theorems, bridge via L_equip [P].
    UPGRADE HISTORY: [P_structural | regime R12] ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ [P] via L_equip.
    """
    N_gen = 3
    N_mult_refs = 16
    ...
    return _result(
        ...
        summary=(
            ...
            'Bridge: L_equip proves ÃƒÅ½Ã‚Â©_sector = |sector|/C_total at '
...
def check_T_canonical():
    ...
    return _result(
        ...
        epistemic='P',
        summary=(
            'Paper 13 Â§9. The admissibility structure is a sheaf of '
...
def check_L_Omega_sign():
    ...
    Paper 13 Â§10.  First quantitative test of the canonical object.

    STATEMENT: The two Î© functionals of Theorem 9.16 have opposite sign
    tendencies, and Î©_inter is identified with negative mutual information:

    (1a) Î©_local > 0 for SOME pairs (L_nc: composition costs more). [P]
    (1b) Î©_local â‰¥ 0 for ALL pairs sharing interfaces. [Operational:
         follows from monotonicity of E; see Prop 9.5(c).]
    (2) Î©_inter â‰¤ 0 in quantum-admissible regime (subadditivity). [P]
    (3) Î©_inter = âˆ’I(A:B) exactly, where I(A:B) is mutual information.
    (4) For pure bipartite states: |Î©_inter| = 2Â·S_ent.
    (5) The Î©_inter gap between entangled and classically correlated
        states with identical marginals = quantum discord.
    (6) The sign constraint Î©_inter â‰¤ 0 is NOT derivable from L1-L5
        alone (the discrete witness in T_canonical has Î©_inter > 0).
        Subadditivity is quantum content, requiring T2.

    PHYSICAL INTERPRETATION:
      Î©_local > 0: composing WHAT at same WHERE â†’ incompatibility
      Î©_inter < 0: correlating same WHAT at different WHERE â†’ entanglement
    ...
    # â”€â”€ (1) Product pure: Î©_inter = 0 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    psi = _zvec(dAB)
    psi[0] = complex(1)
    rho = _outer(psi, psi)
    omega, mi, sab, sa, sb = Omega_inter(rho, dA, dB)
    assert abs(omega) < 1e-12, "Product pure: Î©_inter = 0"

    # â”€â”€ (2) Bell state: Î©_inter = âˆ’2ln2 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    psi_bell = _zvec(dAB)
    ...
    assert abs(omega_bell - (-2 * ln2)) < 1e-10, "Bell: Î©_inter = âˆ’2ln2"
    assert abs(mi_bell - 2 * ln2) < 1e-10, "Bell: I(A:B) = 2ln2"

    # â”€â”€ (3) Partially entangled: Î©_inter = âˆ’2Â·S_ent â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    psi_part = _zvec(dAB)
    ...
    assert abs(omega_part - (-2 * S_ent_expected)) < 1e-10, "Pure: Î© = âˆ’2Â·S_ent"

    # â”€â”€ (4) Classical correlated: same marginals, different Î© â”€â”€â”€â”€â”€â”€â”€â”€
    psi_11 = _zvec(dAB)
    psi_11[3] = complex(1)
    ...
    assert abs(omega_class - (-ln2)) < 1e-10, "Classical: Î©_inter = âˆ’ln2"

    # KEY: same marginals (Prop 9.12), different Î©_inter
    assert abs(sa_bell - sa_class) < 1e-10, "Same local cost at A"
    assert abs(sb_bell - sb_class) < 1e-10, "Same local cost at B"
    assert abs(omega_bell - omega_class) > 0.5, "Different Î©_inter"
    # Gap = quantum discord = ln2
    gap = abs(omega_bell) - abs(omega_class)
    assert abs(gap - ln2) < 1e-10, "Gap = ln2 = quantum discord"

    # â”€â”€ (5) Product mixed: Î©_inter = 0 â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    rho_Am = _diag([0.7, 0.3])
    rho_Bm = _diag([0.6, 0.4])
    rho_prod = _kron(rho_Am, rho_Bm)
    omega_prod, mi_prod, _, _, _ = Omega_inter(rho_prod, dA, dB)
    assert abs(omega_prod) < 1e-10, "Product mixed: Î©_inter = 0"

    # â”€â”€ (6) Subadditivity scan: Î©_inter â‰¤ 0 for random states â”€â”€â”€â”€â”€â”€â”€
    import random
    random.seed(42)
    n_tests = 200
    for _ in range(n_tests):
        ...
        assert omega_r <= 1e-12, f"Subadditivity violation! Î© = {omega_r}"
    ...
    # â”€â”€ (7) Î©_local > 0 (from L_nc witness for comparison) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    from fractions import Fraction
    E_a = Fraction(2)
    E_b = Fraction(3)
    E_ab = Fraction(9)
    Omega_local = E_ab - E_a - E_b  # = 4
    assert Omega_local > 0, "Î©_local > 0 (L_nc)"

    # â”€â”€ (8) Discrete Î©_inter > 0 (pre-quantum allows positive) â”€â”€â”€â”€â”€â”€
    Omega_inter_discrete_x = Fraction(5) - Fraction(2) - Fraction(2)  # = 1
    Omega_inter_discrete_y = Fraction(7) - Fraction(2) - Fraction(2)  # = 3
    assert Omega_inter_discrete_x > 0, "Pre-quantum: Î©_inter can be > 0"
    assert Omega_inter_discrete_y > 0, "Pre-quantum: Î©_inter can be > 0"
    # This proves Î©_inter â‰¤ 0 is NOT a pre-quantum theorem
    ...
    return _result(
        ...
        summary=(
            ...
            'Î©_inter = âˆ’I(A:B) (negative mutual information) in the '
            'quantum-admissible regime. For pure states: |Î©_inter| = 2Â·S_ent. '
            'Sign dichotomy: Î©_local â‰¥ 0 generically (L_nc, composition costs more), '
            'Î©_inter â‰¤ 0 always in quantum regime (subadditivity, correlation saves '
            ...
            'Sign constraint Î©_inter â‰¤ 0 is NOT pre-quantum (discrete witness '
            'has Î©_inter > 0). Subadditivity requires T2.'
        ),
        key_result=(
            'Î©_inter = âˆ’I(A:B); sign dichotomy Î©_local â‰¥ 0 / Î©_inter â‰¤ 0 '
        ...
        artifacts={
            'identification': 'Î©_inter = âˆ’I(A:B) = S(Ï_AB) âˆ’ S(Ï_A) âˆ’ S(Ï_B)',
            'bell_state': {
            ...
            'physical_interpretation': (
                'Î©_local > 0 = measurement incompatibility; '
                'Î©_inter < 0 = capacity-efficient correlation (entanglement)'
            ),
        },
    )
...
def check_L_Gram():
    ...
    Paper 13 Â§9 + Paper 61 Â§5.  Second test of the canonical object.
    ...
    object's channel space:
        ...
        a_ij = âŸ¨v_i, v_jâŸ©  where v_i(e) = demand of sector i on channel e.
    ...
    (3) Î³â‚‚/Î³â‚ = Tr(A) (sum of squared demand norms).
    (4) Generalizes: det = dim(adjoint(SU(N_w))) = N_wÂ² - 1 for any N_w.
    (5) Provides second derivation route to sinÂ²Î¸_W = 3/13 through
    ...
    # â”€â”€ EW channel space (T_channels: d=4) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    m = 3   # dim(su(2))
    n_ch = 4  # 1 bookkeeper + 3 mixers
    x = Fraction(1, 2)  # T27c / T_S0

    # â”€â”€ Sector demand vectors â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Gram matrix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    a11 = _fdot(v1, v1)
    a12 = _fdot(v1, v2)
    a22 = _fdot(v2, v2)
    assert a11 == 1
    ...
    # â”€â”€ Cauchy-Binet decomposition â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    V = [v1, v2]
    det_cb = Fraction(0)
    nonzero_minors = 0
    ...
    # â”€â”€ x-independence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Generalization to SU(N_w) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Î³â‚‚ = Tr(A) connection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    gamma = Fraction(17, 4)
    trace_A = a11 + a22
    assert trace_A == gamma, f"Tr(A) must be Î³â‚‚/Î³â‚ = {gamma}"

    # â”€â”€ Chain to sinÂ²Î¸_W â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    g1, g2 = Fraction(1), gamma
    r_star = (g1 * a22 - g2 * a12) / (g2 * a11 - g1 * a12)
    sin2 = r_star / (1 + r_star)
    assert r_star == Fraction(3, 10)
    ...
    return _result(
        ...
        summary=(
            ...
            f'Chain: Gram matrix â†’ det = 3 â†’ r* = 3/10 â†’ sin2_W = 3/13.'
        ),
        key_result=(
        ...
        artifacts={
            ...
            'x_independence': 'verified at 7 values; algebraic: det = 1Â·(xÂ²+m) - xÂ² = m',
            'generalization': 'det = N_wÂ² - 1 for SU(N_w), verified N_w = 2..6',
...
def check_L_beta():
    """L_beta: Î²-Function Invariances Grounded in Canonical Object.

    Paper 13 Â§10 + Paper 61 Â§4.  Third test of the canonical object.

    STATEMENT: The five structural invariances I1-I5 that uniquely
    determine the Lotka-Volterra Î²-function form (T21) each follow
    from a specific canonical object proposition:

      I1 (Extinction)           â† Prop 9.1 (order ideal: âˆ… âˆˆ Adm, E(âˆ…)=0)
      I2 (Permutation covariance) â† Aut(Î“) (Â§9.7: cost-preserving bijections)
      I3 (Interface additivity)  â† Props 9.9-9.10 (restriction maps: âˆ© over âˆª)
      I4 (Symmetric competition) â† L_Gram (a_ij = âŸ¨v_i,v_jâŸ© symmetric)
      I5 (Quadratic truncation)  â† Prop 9.8 (pairwise structure) + Îµ/C â‰ª 1 (suppression)

    MECHANISM: RG = coarse-graining of distinctions (T20). Each step
    merges distinction sets, changing Î© by exactly one Î” term (Prop 9.8).
    In the continuous limit this produces the bilinear interaction
    w_i Î£_j a_ij w_j. Combined with linear dissipation (individual
    ...
    STATUS: [P] â€” all grounding propositions are [P].
    """
    # â”€â”€ Verify I1: Extinction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    # E(âˆ…) = 0 and Î”(âˆ…, S) = 0 for any S
    C = Fraction(10)
    E = {
        frozenset():          Fraction(0),
    ...
    for S in [frozenset(['a']), frozenset(['b']), frozenset(['c'])]:
        ...
        assert delta_empty == 0, "Î”(âˆ…, S) must be 0"

    # â”€â”€ Verify I4: Symmetry of Î” â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    for S1, S2 in [
        ...
        assert D_12 == D_21, f"Î” must be symmetric"

    # â”€â”€ Verify I5: Prop 9.8 exact refinement (algebraic identity) â”€â”€â”€â”€
    # Î©({a},{b},{c}) = Î©({aâˆªb},{c}) + Î”(a,b) â€” identity for ANY E
    ...
    # All pairwise Î” > 0: L_nc holds for all pairs

    # â”€â”€ Verify mechanism: fixed point from Gram matrix â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    x = Fraction(1, 2)
    m = 3
    gamma = Fraction(17, 4)
    a11, a12 = Fraction(1), x
    ...
    # Stability: Jacobian J = -diag(w*) Â· A
    ...
    grounding = {
        ...
        'I2_permutation': 'Aut(Gamma) (Â§9.7)',
    ...
    return _result(
        name='L_beta: Î²-Function Grounded in Canonical Object',
        tier=0,
        epistemic='P',
        summary=(
            'I1-I5 invariances grounded in canonical object: '
            'I1â†Prop9.1, I2â†Aut(Î“), I3â†Props9.9-9.10, '
            'I4â†L_Gram, I5â†Prop9.8. '
            ...
            f'sinÂ²Î¸_W=3/13. Stability: tr(J)={tr_J}<0, det(J)={det_J}>0.'
        ),
        key_result='Î²-function form = canonical object response to coarse-graining',
...
def check_L_gen_path():
    ...
    assert all(Q[g] < Q[g + 1] for g in range(N - 1)), "Must be strictly increasing"

    # Telescoping
    assert Q[2] - Q[0] == (Q[1] - Q[0]) + (Q[2] - Q[1]), "Telescoping"

    # Path cost = sum of consecutive differences
    diffs = [Q[g + 1] - Q[g] for g in range(N - 1)]
    assert all(d > 0 for d in diffs)
    assert sum(diffs) == Q[2] - Q[0]
    ...
    assert abs(lhs - rhs) < 1e-15, "FN factorization through gen 2"
...
def check_T_capacity_ladder():
    ...
    assert Q == [2, 5, 9], f"Q = {Q}"
    assert q_B == [7, 4, 0], f"q_B = {q_B}"
    ...
    return _result(
        ...
        summary=(
            ...
            'These are the Froggatt-Nielsen suppression charges for the '
            'bookkeeper channel of the mass matrix.'
        ),
        key_result=f'q_B = {tuple(q_B)} [P]',
        dependencies=['T7', 'T_kappa', 'T_eta'],
    )
...
def check_L_D2q():
    ...
    for kappa in range(0, 6):
        ...
        D2q = q[0] - 2 * q[1] + q[2]
        assert D2q == -eps, f"kappa={kappa}: D2q={D2q}, expected {-eps}"
...
def check_L_H_curv():
    ...
    assert best_h == (0, 1, 0), f"Expected (0,1,0), got {best_h}"
    assert len(all_optimal) == 1, f"Must be unique, got {all_optimal}"
    ...
    assert l2_h[0] + l2_h[1] >= 1 and l2_h[1] + l2_h[2] >= 1
    assert not all(h.denominator == 1 for h in l2_h), "l2 solution is fractional"
...
def check_T_q_Higgs():
    """T_q_Higgs: Higgs Channel Charges [P_structural].

    STATEMENT: The Higgs channel FN charges are q_H = q_B + h = (7, 5, 0),
    where h = (0, 1, 0) is the interior bump from L_H_curv.

    The Higgs VEV lives in mixer channel M2 (T3 = -1/2, down-type).
    It couples directly to down-type quarks (c_Hd = 1) and must
    propagate + conjugate to couple to up-type (c_Hu = x^3).
    """
    q_B = [7, 4, 0]
    h = [0, 1, 0]
    q_H = [q_B[g] + h[g] for g in range(3)]
    assert q_H == [7, 5, 0], f"q_H = {q_H}"

    # The Cabibbo angle lives in Dq_H(2) = q_H(2) - q_B(2) = 1
    Delta_q = q_H[1] - q_B[1]
    assert Delta_q == 1, "Cabibbo angle source: gen-2 Higgs correction = 1"
    ...
    return _result(
        ...
        epistemic='P_structural',
        summary=(
            f'q_H = q_B + h = {tuple(q_H)}. Interior bump h=(0,1,0) from L_H_curv. '
            f'Cabibbo angle source: Dq_H(gen2) = {Delta_q}. '
            'Higgs VEV in M2 (T3=-1/2). Down-type: direct (c_Hd=1). '
            'Up-type: propagate + conjugate (c_Hu = x^3).'
        ),
        key_result=f'q_H = {tuple(q_H)} [P_structural]',
        dependencies=['T_capacity_ladder', 'L_H_curv', 'T_Higgs'],
    )


def check_L_holonomy_phase():
    """L_holonomy_phase: phi = pi/4 from SU(2) Orthogonal-Generator Holonomy [P_structural].
    ...
    PROOF:
      (a) The 3 Pauli matrices define 3 mutually orthogonal points on S^2.
      (b) The spherical triangle they form has side length s = pi/2.
      (c) Spherical law of cosines: cos(A) = cos(s)/(1+cos(s)).
          For s=pi/2: cos(A) = 0, so A = pi/2.
      (d) Spherical excess: E = 3A - pi = 3(pi/2) - pi = pi/2.
      (e) Holonomy for spin-j: j * E. For fundamental (j=1/2): E/2 = pi/4.

    BRIDGE: Generation cycle visits orthogonal mixer channels (consistent
    with framework but not independently proved).
    """
    s = _math.pi / 2
    ...
    assert abs(A - _math.pi / 2) < 1e-10, f"Angle = {A}, expected pi/2"
    assert abs(E - _math.pi / 2) < 1e-10, f"Excess = {E}, expected pi/2"
    assert abs(holonomy - _math.pi / 4) < 1e-10, f"Holonomy = {holonomy}, expected pi/4"
    ...
    for s_test in [_math.pi / 6, _math.pi / 4, _math.pi / 3,
        ...
        Et = 3 * At - _math.pi
        ht = Et / 2
        if abs(ht - _math.pi / 4) < 0.01:
            found = True
            assert abs(s_test - _math.pi / 2) < 0.01, "Only s=pi/2 gives pi/4"
    ...
    return _result(
        ...
        epistemic='P_structural',
        summary=(
            'Orthogonal-generator triangle: 3 Pauli matrices on S^2 at '
            'mutual angle pi/2. Equilateral spherical triangle with '
            f'side={s:.4f}, angle={A:.4f}=pi/2, excess={E:.4f}=pi/2. '
            f'Fundamental holonomy = E/2 = {holonomy:.4f} = pi/4. '
            'Bridge: generation cycle visits orthogonal mixer channels.'
        ),
        key_result='phi = pi/4 [P_structural]',
        dependencies=['T_gauge'],
    )


def check_L_adjoint_sep():
    """L_adjoint_sep: Delta_k = 3 = dim(adj SU(2)) [P].

    STATEMENT: The CKM matrix depends only on Delta_k = k_u - k_d
    (gauge equivalence class). Maximal separation in SU(2) adjoint
    (T3 eigenvalues {-1, 0, +1}) gives Delta_k = dim(adj) = 3.

    PROOF:
      (a) Phase per matrix element: exp(i*phi*(g-h)*k/3).
      (b) V_CKM = U_u^dag U_d depends on k_u - k_d only.
      (c) Verified computationally: all (k_u, k_d) with same Delta_k
          give identical CKM.
      (d) SU(2) adjoint has dim=3, T3 in {-1,0,+1}.
          Higgs conjugation H->H_tilde maps highest to lowest weight:
          Delta_k = dim(adjoint) = 3.
    """
    # Verify gauge equivalence: CKM depends only on Delta_k
    x = 0.5
    phi = _math.pi / 4
    q_B = [7, 4, 0]; q_H = [7, 5, 0]; c_Hu = x ** 3
    Delta_k = 3

    # Compute CKM for different global shifts n (k_u=3+n, k_d=0+n, k_H=0+n)
    Vus_vals = []
    for n in [0, 1, -2, 5]:
        k_u = Delta_k + n; k_d = 0 + n; k_H = 0 + n
        M_u = _build_two_channel(q_B, q_H, phi, k_u, k_H, 1.0, c_Hu)
        M_d = _build_two_channel(q_B, q_H, phi, k_d, k_H, 1.0, 1.0)
        _, Vu = _diag_left(M_u)
        _, Vd = _diag_left(M_d)
        Vckm = _mm(_dag(Vu), Vd)
        Vus_vals.append(abs(Vckm[0][1]))

    # All should be identical
    for v in Vus_vals:
        assert abs(v - Vus_vals[0]) < 1e-10, "Gauge equivalence violated"

    # dim(adj SU(2)) = 3
    dim_adj = 2 ** 2 - 1  # N^2 - 1 for SU(N)
    assert dim_adj == 3
    assert Delta_k == dim_adj

    return _result(
        name='L_adjoint_sep: Delta_k = 3 (Gauge Equivalence)',
        tier=3,
        epistemic='P',
        summary=(
            f'CKM depends only on Delta_k = k_u - k_d = {Delta_k}. '
            'Gauge equivalence verified: 4 representative (k_u,k_d) pairs '
            'with Delta_k=3 all give identical CKM. '
            f'dim(adj SU(2)) = N^2-1 = {dim_adj} = Delta_k. '
            'Higgs conjugation maps highest to lowest adjoint weight.'
        ),
        key_result=f'Delta_k = {Delta_k} = dim(adj SU(2)) [P]',
        dependencies=['T_gauge'],
    )


def check_L_channel_crossing():
    """L_channel_crossing: c_Hu/c_Hd = x^3 [P_structural].

    STATEMENT: The ratio of Higgs coupling coefficients for up-type
    vs down-type quarks is c_Hu/c_Hd = x^3 = 1/8.

    DERIVATION:
      (a) Higgs VEV lives in M2 (down-type channel). c_Hd = 1 (direct).
      (b) For up-type: Higgs must propagate M2 -> B -> M1.
          Two boundary crossings, each with overlap x (from L_Gram).
          Propagation cost: x^2.
      (c) Conjugation H -> H_tilde: one enforcement step costs x.
          (anti-fund ~ fund for SU(2), single epsilon contraction.)
      (d) Total: c_Hu/c_Hd = x^2 * x = x^3 = 1/8.
    """
    x = Fraction(1, 2)
    propagation = x ** 2  # two boundary crossings
    conjugation = x       # one enforcement step
    ratio = propagation * conjugation
    assert ratio == x ** 3 == Fraction(1, 8)
    ...
    return _result(
        ...
        epistemic='P_structural',
        summary=(
            f'c_Hu/c_Hd = x^3 = {float(ratio)}. '
            f'Propagation M2->B->M1: x^2 = {float(propagation)} (two crossings). '
            f'Conjugation H->H_tilde: x = {float(conjugation)} (one step). '
            'Soft spot: conjugation cost x vs x^2 empirically degenerate at 3%.'
        ),
        key_result='c_Hu/c_Hd = x^3 = 1/8 [P_structural]',
        dependencies=['L_Gram', 'T_q_Higgs'],
    )


def check_T_CKM():
    """T_CKM: Zero-Parameter CKM Matrix Prediction [P_structural].

    STATEMENT: From derived structural inputs alone (zero scanned parameters,
    zero fitted parameters), the CKM mixing matrix is predicted with
    all 6 magnitudes within 5% of experiment.
    ...
    INPUTS (all derived):
      ...
      q_H = (7, 5, 0)   [T_q_Higgs]
      phi = pi/4         [L_holonomy_phase]
      Delta_k = 3        [L_adjoint_sep]
      c_Hu = x^3 = 1/8  [L_channel_crossing]

    PREDICTIONS vs EXPERIMENT:
      theta_12: 13.50 vs 13.04 (+3.5%)
      theta_23: 2.32  vs 2.38  (-2.6%)
      theta_13: 0.209 vs 0.201 (+3.9%)
      |V_us|:   0.2334 vs 0.2257 (+3.4%)
      |V_cb|:   0.0404 vs 0.0410 (-1.4%)
      |V_ub|:   0.00364 vs 0.00382 (-4.6%)
      J_CKM:   3.33e-5 vs 3.08e-5 (+8.1%)

    SM COMPARISON: 4 free parameters -> 4 CKM observables (zero predictions).
    FCF: 0 free parameters -> 6+ observables (all predictions).

    KNOWN MISS: delta_CP = 85 vs 68 deg (near-maximal CP violation,
    correlated with phi = pi/4 giving sin(delta) ~ 1).
    """
    x = 0.5
    phi = _math.pi / 4
    q_B = [7, 4, 0]; q_H = [7, 5, 0]
    ...
    # Verify all magnitudes within 5%
    checks = [
    ...
    assert within_5 == 6, f"Expected 6/6 within 5%, got {within_5}/6"

    # Hierarchy
    assert a['theta12'] > a['theta23'] > a['theta13'], "Hierarchy violated"
    # Correct CP sign
    assert J > 0, "Jarlskog must be positive"
    # J within 10%
    assert abs(J / exp['J'] - 1) < 0.10, f"J error: {(J/exp['J']-1)*100:.1f}%"
    ...
    return _result(
        ...
        summary=(
            ...
            'Hierarchy correct. CP sign correct. '
            'Known miss: delta_CP = 85 vs 68 deg (near-maximal). '
            'SM: 4 free params -> 4 observables. FCF: 0 -> 6+.'
        ),
        key_result='CKM 6/6 within 5%, zero free parameters [P_structural]',
        dependencies=[
            'T27c', 'T_capacity_ladder', 'T_q_Higgs',
            'L_holonomy_phase', 'L_adjoint_sep', 'L_channel_crossing',
        ],
        artifacts={
            'theta12': a['theta12'], 'theta23': a['theta23'],
            'theta13': a['theta13'],
...
def check_T_PMNS_partial():
    """T_PMNS_partial: PMNS Structural Wall [P_structural | open].

    STATEMENT: Extending the CKM derivation to the lepton sector
    ...
    assert wu[0] > 0 or wu[1] > 1e-20, "Up-type mass matrix must have structure"
    ...
    assert abs(wn[0]) < 1e-8, "Neutrino mass matrix is near rank-1"
    assert abs(wn[1]) < 1e-8, "Second eigenvalue also near zero"
    assert wn[2] > 1.0, "Largest eigenvalue is O(1)"

    return _result(
        name='T_PMNS_partial: PMNS Structural Wall',
        tier=3,
        epistemic='P_structural | open',
        summary=(
            'PMNS extension hits structural wall. Neutrino FN texture with '
            'small charges gives rank-1 mass matrix: eigenvalues '
            f'({wn[0]:.1e}, {wn[1]:.1e}, {wn[2]:.1f}). '
            'theta_12 solver-dependent (67 deg spread under perturbation). '
            'theta_23 ~ 44 deg and theta_13 ~ 8 deg are solver-stable '
            '(determined by non-degenerate sector). '
            'ROOT CAUSE: large PMNS angles require near-democratic M_nu -> '
            'small FN charges -> rank deficiency. '
            'Neutrino mass mechanism likely differs from quark/lepton Yukawas. '
            'Framework correctly predicts PMNS >> CKM from no-color, '
            'but cannot derive theta_12 from FN texture alone.'
        ),
        key_result='PMNS structural wall: theta_12 undetermined in FN texture [open]',
        dependencies=['T_CKM'],
        artifacts={
            'nu_eigenvalues': [float(w) for w in wn],
            'solver_stable': ['theta_23 ~ 44 deg', 'theta_13 ~ 8 deg'],
            'solver_dependent': ['theta_12 (67 deg spread)'],
            'structural_wall': 'rank-1 neutrino mass matrix from small FN charges',
        },
    )
...
THEOREM_REGISTRY = {
    ...
    'T_CKM':            check_T_CKM,
    'T_PMNS_partial':   check_T_PMNS_partial,
...
def display():
    ...
    print(f"{'=' * W}")
    print(f"  FCF THEOREM BANK -- v4.3.0  (Flavor Mixing)")
    print(f"{'=' * W}")
